...
class NewCartSearch extends Component {
    ...
    callSearchAPi = (check, history, search) => {
        Axios.get(`${Config.prod}/api/oc/${check ? search : `search?q=${search}`}`, {
            params: {
                token: loadUserToken()
            }
        }).then(({ data }) => {
                    ...
                    <Row >
                        {
                            items.map((item, index) => (
                                <Col className="col-12 col-sm-6 col-lg-4  mb-2" key={index}>
                                    <Card style={{ borderRadius: '5px', cursor: 'pointer' }} onClick={() => this.renderCart(item)} >
                                        <Card.Body>
                                            <div>
                                                <h6  ><b>Order Confirmation Id:</b> {item.orderconfirmationid}</h6>
                                                <h6>
                                                    <b>Job Name: </b>{item.jobname}
                                                </h6>
                                                <h6  ><b>Cost Center Id: </b>{item.costcenterid}</h6>
                                                <h6  ><b>Contact Info: </b>{item.contactinfo}</h6>
                                            </div>
                                        </Card.Body>
                                    </Card>
                                </Col>
