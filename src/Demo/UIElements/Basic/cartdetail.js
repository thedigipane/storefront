...
class CartDetailComponent extends Component {
  ...
  checkout = () => {
    ...
    Axios.put(
      config.prod + "/api/cart/" + item.idcart + "/checkout",
      { ocId: item.idPO, title, description },
      {
        headers: {
          "Content-Type": "application/json",
        },
  ...
  updateCartProduct = (item, index) => {
    ...
    console.log(item);
    Axios.put(
      config.prod + `/api/cart/${item.idcart}/product`,
      {
                ...
                <Card style={{ borderRadius: "5px", cursor: "pointer" }}>
                  <Row className="p-2 pt-3" noGutters>
                    <Col className="col-5 p-2">
                      {
                        <img
                          alt={item.details && item.details.imagedescription}
                          src={item.details && item.details.images[0]}
                          className="img-fluid"
                        />
                      }
                    </Col>
                    <Col className="col-7 pt-3 text-right pr-3">
                      ...
                      <h6>
                        Company:{" "}
                        {item.details &&
                          item.details.componentDetails.companyname}
                      </h6>
                      <h6>
                        Model:{" "}
                        {item.details &&
                          item.details.componentDetails.mfgmodelnumber}
                      </h6>
                    </Col>
                    <Col className="col-12 px-3 mt-3">
                      <h6>
                        <b>Available: </b>
                        {item.details && item.details.availablequantity}
                      </h6>
                    </Col>
                  </Row>
                  <Card.Body>
                    <Row noGutters>
                      <Col className="col-8 col-sm-12   col-xl-8 ">
                        <div id="incdec">
                          ...
                          <input
                            ...
                            name="quatity"
                            value={item.quantity}
                            onChange={(e) => this.onInputValue(item, index)}
                          />
                          <Button
                            size="sm"
                      ...
                      <Col className="col-12 col-sm-12   col-xl-12  text-right">
                        <Button
                          size="sm"
                          onClick={() => this.updateCartProduct(item, index)}
                        >
                          Update
                        </Button>
                        <Button
                          size="sm"
                          variant="danger"
                          onClick={() => this.deleteStock(item.idstock, index)}
                        >
                          Delete
                        </Button>
                      </Col>
            ...
            <Form>
              <FormGroup>
                <FormControl
                  placeholder="Tilte"
                  name="title"
                  onChange={(e) => this.rendertextinput(e)}
                />
              </FormGroup>
              <FormGroup>
                <FormControl
